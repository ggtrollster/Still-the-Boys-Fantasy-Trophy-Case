(async function () {
  ...
  try {
    ...
    data = await res.json();
    seasons = Array.isArray(data.seasons) ? data.seasons.slice() : [];
  } catch (e) {
  ...
  function goldStops(){
    return [{off:0,color:"#ffd76a"},{off:50,color:"#f7b500"},{off:100,color:"#c88700"}];
  }
  function svgForStyle(style, colorHex, key) {
    ...
    switch ((style||"classic").toLowerCase()) {
      case "classic":
        return `<svg class="trophy-svg" viewBox="0 0 200 200" aria-hidden="true">
  ${g}
  <g>
  ...
  function renderLeaderboard(seasons) {
    ...
    const rows = Object.entries(getCountsFromSeasons(seasons)).map(([name, c]) => ({ name, ...c }));
  ...
  function openModal(season) {
    ...
    if (titleEl) titleEl.textContent = `Season ${season.year}`;
    if (subtitleEl) subtitleEl.textContent = season.champion ? `Champion: ${season.champion}${season.team ? " — " + season.team : ""}` : "";
